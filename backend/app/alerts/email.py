...
def build_signal_email(
    ...
    def signal_rows(signal_list):
        ...
        for s in signal_list:
            ...
            ema20 = s.get("ema20_level") or 0.0

            if status == "in_zone":
                entry_note = f"<div style='color:#16a34a;font-size:11px;margin-top:2px;'>Buy near ₹{ema20:,.2f}</div>"
            else:
                entry_note = f"<div style='color:#64748b;font-size:11px;margin-top:2px;'>Wait for ₹{ema20:,.2f}</div>"
