...
def calculate_technical_score(df: pd.DataFrame, timeframe: str = "D") -> dict:
    ...
    momentum_6m = (
        ((price / df["Close"].iloc[-127] - 1) * 100) if len(df) >= 127 else None
    )
    momentum_12m = (
        ((price / df["Close"].iloc[-253] - 1) * 100) if len(df) >= 253 else None
    )
        ...
        if timeframe == "D":
            ...
            if pd.notna(rsi) and pd.notna(prev_rsi):
                ...
                if recovering and fresh_ema_cross:
                    score += (
                        15  # capped at RSI budget; EMA cross already scored separately
                    )
