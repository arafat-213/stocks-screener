...
def calculate_fundamental_score(info: dict, fund_cache=None) -> float:
    """
    Calculates fundamental sub-score (Max 30 pts)
    """
    score = 0

    # 1. Market Cap Score (Max 5 pts)
    mcap = None
    if fund_cache and fund_cache.market_cap is not None:
        mcap = fund_cache.market_cap
    else:
        mcap = to_float(info.get("marketCap"))

    if mcap is not None:
        if mcap > 100_000_000_000:  # > 10k Cr
            score += 5
        elif mcap > 20_000_000_000:  # > 2k Cr
            score += 2

    # 2. P/E Score (Max 10 pts)
    pe = None
    if fund_cache and fund_cache.pe_ratio is not None:
        pe = fund_cache.pe_ratio
    else:
    ...
    if fund_cache and fund_cache.de_ratio is not None:
        ...
        ):  # Handle percentage format (e.g. 50 instead of 0.5)
            de = de / 100.0
...
def calculate_technical_score(
    ...
    # Safety check
    if i < 0 or i >= len(df):
        return calculate_technical_score(pd.DataFrame())
    ...
    momentum_1m = ((price / df["Close"].iloc[i - 21] - 1) * 100) if i >= 21 else None
    momentum_3m = ((price / df["Close"].iloc[i - 63] - 1) * 100) if i >= 63 else None
    momentum_6m = ((price / df["Close"].iloc[i - 126] - 1) * 100) if i >= 126 else None
    momentum_12m = ((price / df["Close"].iloc[i - 252] - 1) * 100) if i >= 252 else None
