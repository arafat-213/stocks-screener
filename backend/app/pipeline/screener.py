...
def check_profitability_streak(financials) -> bool:
    ...
    try:
        if financials is None or financials.empty or len(financials.columns) < 3:
            return False
...
def fetch_and_cache_deep_fundamentals(symbols: list[str], db_session: Session):
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        for symbol in batch:
            ...
            cache_entry = (
                db_session.query(FundamentalCache)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    roce = None
                    if (
                        ebit_row is not None
                        and assets_row is not None
                        and liab_row is not None
                    ):
                        try:
                    ...
                    logger.error(
                        f"Attempt {attempt + 1} failed for {symbol}: {e}. Retrying in {wait_time:.2f}s..."
                    )
            ...
            try:
                ...
                if de_ratio is not None:
                    ...
                    if normalized_de > de_limit:
                        ...
                        logger.info(
                            f"{symbol} failed D/E check: {normalized_de} > {de_limit} (Sector: {sector})"
                        )
                else:
                    normalized_de = None
                ...
                fund_data = (
                    db_session.query(FundamentalData)
                    .filter(
                        FundamentalData.symbol == symbol, FundamentalData.date == today
                    )
                    .first()
                )
