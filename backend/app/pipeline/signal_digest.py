            ...
            "pullback_entry_zone": (
                {
                    ...
                    "tolerance_high": round(
                        sig.ema20_level * (1 + config.pullback_tolerance_pct / 100), 2
                    ),
                }
                if sig.ema20_level and sig.ema_signal == "bullish_cross"
                else None
            ),
            "stop_reference": (
                round(sig.close_price - config.atr_multiplier * sig.atr, 2)
                if sig.close_price and sig.atr
                else None
            ),
        }
