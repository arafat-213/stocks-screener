...
def create_entry(data: TradeEntryCreate, db: Session = Depends(get_db)):
    ...
    if data.watchlist_id:
        wl = (
            ...
            .filter(models.Watchlist.id == data.watchlist_id)
            .first()
        )
        if wl:
            wl.status = "entered"
...
def get_open_trades(db: Session = Depends(get_db)):
    ...
    for t in trades:
        ...
        if current_price > 0 and t.stop_loss is not None:
            dist_to_stop = round(
                ((current_price - t.stop_loss) / current_price) * 100, 2
            )
        ...
        if current_price > 0 and t.target is not None:
            dist_to_target = round(
                ((t.target - current_price) / current_price) * 100, 2
            )
...
def close_trade(trade_id: int, data: TradeCloseRequest, db: Session = Depends(get_db)):
    trade = (
        db.query(models.TradeJournal).filter(models.TradeJournal.id == trade_id).first()
    )
    if not trade:
    ...
    if trade.entry_price > 0:
        trade.return_pct = round(
            ((data.exit_price - trade.entry_price) / trade.entry_price) * 100, 2
        )
...
def get_journal_stats(db: Session = Depends(get_db)):
    ...
    avg_return = (
        round(sum([t.return_pct or 0 for t in closed_trades]) / total_trades, 2)
        if total_trades > 0
        else 0
    )
