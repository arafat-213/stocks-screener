...
from app.backtest.engine import ROUND_TRIP_COST_PCT
...
def get_portfolio_summary(db: Session = Depends(get_db)):
    ...
    if not portfolio:
        return {"status": "no_portfolio", "message": "No active paper portfolio. Run a pipeline cycle to initialise."}
    ...
    return [
        {
            ...
            "closeness_pct": round(p.pending_highest_closeness_pct, 2) if p.pending_highest_closeness_pct != 999.0 else None
        } for p in pending
    ]
...
def get_open_positions(db: Session = Depends(get_db)):
    ...
    for pos in positions:
        ...
        unrealised_pct = ((current_price - pos.entry_price) / pos.entry_price * 100) if current_price else None
