...
def _build_screen_response(
    ...
    setup = (
        compute_trade_setup(tech, capital=capital, risk_pct=risk_pct) if tech else None
    )
...
def get_screen_results(
    ...
    capital: float = Query(
        default=1_000_000.0, description="Your trading capital for position sizing"
    ),
    risk_pct: float = Query(
        default=3.0, ge=0.5, le=10.0, description="% of capital to risk per trade"
    ),
    ...
    if not live:
        ...
        if not latest_run:
            ...
            db_results = (
                ...
                .outerjoin(
                    models.FundamentalCache,
                    models.Stock.symbol == models.FundamentalCache.symbol,
                )
                .outerjoin(
                    latest_signal_sub, models.Stock.symbol == latest_signal_sub.c.symbol
                )
                .outerjoin(
                    models.TechnicalSignal,
    ...
    if (live or not results) and slug in SCREEN_REGISTRY:
        ...
        try:
            live_data = screen_meta["fn"](
                db
            )  # Returns list of (symbol, score) or symbols
            if live_data:
                ...
                is_tuple_like = hasattr(first_item, "__iter__") and not isinstance(
                ...
                if live_symbols:
                    ...
                    enriched = (
                        ...
                        .outerjoin(
                            latest_signal_sub,
                            models.Stock.symbol == latest_signal_sub.c.symbol,
                        )
                        .outerjoin(
                            models.TechnicalSignal,
                            (models.Stock.symbol == models.TechnicalSignal.symbol)
                            & (models.TechnicalSignal.timeframe == "D")
                            & (
                                models.TechnicalSignal.date
                                == latest_signal_sub.c.max_date
                            ),
                        )
                        .all()
                    )
            ...
            raise HTTPException(
                status_code=500, detail=f"Error executing screen: {str(e)}"
            )
