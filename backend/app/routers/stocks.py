...
def get_top_stocks(db: Session = Depends(get_db)):
    scores = (
        ...
        .order_by(desc(TechnicalSignal.entry_score))
        .limit(20)
        .all()
    )
    return [
...
def get_stock_detail(symbol: str, db: Session = Depends(get_db)):
    ...
    history_signals = (
        db.query(TechnicalSignal)
        .filter(
            TechnicalSignal.symbol == clean_symbol, TechnicalSignal.timeframe == "D"
        )
    ...
    fund_data = (
        ...
        .order_by(desc(FundamentalData.date))
        .first()
    )
    ...
    fundamentals = {
        ...
        "debt_equity": fund_cache.de_ratio
        if fund_cache
        else (fund_data.debt_equity if fund_data else None),
        "sector": fund_cache.sector
        if fund_cache
        else (stock.sector if stock else None),
        "eps_growth": fund_data.eps_growth if fund_data else None,
        "market_cap": fund_data.market_cap
        if fund_data
        else (stock.market_cap if stock else None),
    }
...
def get_pipeline_status(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "scored": run.stocks_scored,
    }


@router.post("/alerts/trigger")
def trigger_alerts_manual(date: str | None = None, db: Session = Depends(get_db)):
