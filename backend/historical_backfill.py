...
# Constants
CACHE_DIR = os.environ.get("CACHE_DIR", str(Path(__file__).resolve().parent.parent / "data"))
...
def calculate_historical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['pct_from_52w_high'] = ((df['Close'] - df['week52_high']) / df['week52_high']) * 100
    ...
    if 'MACD_12_26_9' in df.columns and 'MACDs_12_26_9' in df.columns:
        df['macd_cross_up'] = (df['MACD_12_26_9'] > df['MACDs_12_26_9']) & (df['MACD_12_26_9'].shift(1) <= df['MACDs_12_26_9'].shift(1))
        df['macd_cross_down'] = (df['MACD_12_26_9'] < df['MACDs_12_26_9']) & (df['MACD_12_26_9'].shift(1) >= df['MACDs_12_26_9'].shift(1))
    else:
...
def generate_signals(df: pd.DataFrame, symbol: str, timeframe: str) -> list:
    ...
    for i in range(1, len(valid_df)):
        ...
        if timeframe == 'D':
            ...
            adx = row.get('ADX_14')
            if pd.notna(adx):
                if adx >= 35: trend_pts += 3
                elif adx >= 25: trend_pts += 2
                elif adx >= 20: trend_pts += 1
            momentum_3m = row.get('momentum_3m')
            if pd.notna(momentum_3m):
                if momentum_3m > 15: trend_pts += 2
                elif momentum_3m > 5: trend_pts += 1
        ...
        signal_dict = {
            ...
            "momentum_6m": float(row.get('momentum_6m')) if pd.notna(row.get('momentum_6m')) else None,
            "momentum_12m": float(row.get('momentum_12m')) if pd.notna(row.get('momentum_12m')) else None,
            "adx": float(row.get('ADX_14')) if pd.notna(row.get('ADX_14')) else None,
            "above_200ema": bool(row.get('above_200ema')) if pd.notna(row.get('above_200ema')) else None,
            "ema_slope_20": float(row.get('ema_slope_20')) if pd.notna(row.get('ema_slope_20')) else None,
            ...
            "pct_from_52w_high": float(row.get('pct_from_52w_high')) if pd.notna(row.get('pct_from_52w_high')) else None,
            "pct_from_52w_low": float(row.get('pct_from_52w_low')) if pd.notna(row.get('pct_from_52w_low')) else None,
            "week52_high": float(row.get('week52_high')) if pd.notna(row.get('week52_high')) else None,
            "week52_low": float(row.get('week52_low')) if pd.notna(row.get('week52_low')) else None,
...
def backfill_history():
    ...
    logger.info(f"Found {total_files} symbols in cache. Starting historical backfill...")
    ...
    try:
        for idx, file_path in enumerate(parquet_files, 1):
            ...
            existing_count = db.query(func.count(TechnicalSignal.id)).filter(
            ...
            if existing_count > 500: # Assuming they already have >2 years of history
                logger.info(f"[{idx}/{total_files}] Skipping {symbol} - already has {existing_count} daily signals.")
                continue
            ...
            try:
                ...
                db.query(TechnicalSignal).filter(TechnicalSignal.symbol == symbol).delete()
                
                # Bulk save
                db.bulk_save_objects(all_signals)
                db.commit()
                
                logger.info(f"  -> Inserted {len(signals_d)} D, {len(signals_w)} W, {len(signals_m)} M records.")
