    ...
    try:
        ...
        for i, target_date in enumerate(dates, 1):
            ...
            if done_count > (total_signals * 0.5):
                logger.info(
                    f"[{i}/{total_dates}] Skipping {target_date} - already has {done_count}/{total_signals} RS scores."
                )
                continue

            logger.info(
                f"[{i}/{total_dates}] Computing RS ranks for {target_date} ({total_signals} symbols)..."
            )
            ...
            if not signals:
                logger.warning(
                    f"  -> No signals with 12m momentum found for {target_date}."
                )
                continue
