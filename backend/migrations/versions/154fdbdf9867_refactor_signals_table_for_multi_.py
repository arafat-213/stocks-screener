...
def upgrade() -> None:
    ...
    op.add_column(
        "technical_signals", sa.Column("timeframe", sa.String(length=1), nullable=True)
    )
    op.add_column(
        "technical_signals", sa.Column("is_bullish", sa.Boolean(), nullable=True)
    )
    op.add_column(
        "technical_signals", sa.Column("rsi_signal", sa.String(), nullable=True)
    )
    op.add_column(
        "technical_signals", sa.Column("scored_at", sa.DateTime(), nullable=True)
    )

    # 3. Backfill
    op.execute(
        "UPDATE technical_signals SET timeframe = 'D', is_bullish = COALESCE(ema_signal = 'bullish', FALSE), scored_at = date"
    )
    ...
    op.create_unique_constraint(
        "uq_symbol_date_tf", "technical_signals", ["symbol", "date", "timeframe"]
    )
