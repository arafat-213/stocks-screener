...
def upgrade() -> None:
    ...
    ts_columns = [col["name"] for col in inspector.get_columns("technical_signals")]
    if "is_consolidating" not in ts_columns:
        op.add_column(
            "technical_signals",
            sa.Column("is_consolidating", sa.Boolean(), nullable=True),
        )

    ts_indexes = [idx["name"] for idx in inspector.get_indexes("technical_signals")]
    if "ix_ts_above_200ema" in ts_indexes:
        op.drop_index("ix_ts_above_200ema", table_name="technical_signals")
    if "ix_ts_rs_score" in ts_indexes:
