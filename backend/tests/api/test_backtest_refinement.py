...
def test_get_backtest_trades_sorting(client, db):
    ...
    response = client.get(
        f"/api/backtest/{run_id}/trades?sort_by=return_pct&sort_dir=desc"
    )
    assert response.status_code == 200
    ...
    response = client.get(
        f"/api/backtest/{run_id}/trades?sort_by=return_pct&sort_dir=asc"
    )
    assert response.status_code == 200
    trades = response.json()["trades"]
