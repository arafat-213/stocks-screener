...
from app.db.models import Stock, TechnicalSignal, Watchlist, FundamentalCache
...
def test_add_to_watchlist_duplicate(client, db):
    ...
    signal = TechnicalSignal(
        symbol=symbol, date=datetime(2026, 5, 20), timeframe='D', entry_score=70.0, close_price=3500.0, atr=70.0
    )
    db.add(signal)
