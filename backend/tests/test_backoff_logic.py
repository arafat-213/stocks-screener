...
def test_needs_cache_refresh():
    ...
    # Case 3: Old cache
    cache = FundamentalCache(symbol="TEST", last_updated=seven_days_ago - datetime.timedelta(days=1))
    assert needs_cache_refresh(cache, seven_days_ago) == True
    
    # Case 4: Recent cache, no backoff
    cache = FundamentalCache(symbol="TEST", last_updated=datetime.datetime.utcnow() - datetime.timedelta(days=1), cache_version=1)
