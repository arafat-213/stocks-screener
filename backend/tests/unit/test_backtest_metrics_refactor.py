...
def test_benchmark_slicing_logic_simulation():
    ...
    benchmark_df = pd.DataFrame(
        {"Close": [100.0] * len(benchmark_dates)}, index=benchmark_dates
    )
    ...
    trades = [
        TradeResult(
            symbol="T.NS",
            sector="S",
            signal_date=datetime.date(2023, 1, 5),
            entry_date=datetime.date(2023, 1, 6),
            exit_date=datetime.date(2023, 1, 15),
            exit_reason="hp",
            signal_score=70.0,
            entry_price=100.0,
            exit_price=105.0,
    ...
    for i in range(1, 20):
        ...
        trades_up.append(
            TradeResult(
                symbol=f"T{i}.NS",
                sector="S",
                signal_date=benchmark_dates[0].date(),
                entry_date=benchmark_dates[0].date(),
                exit_date=benchmark_dates[i].date(),
                exit_reason="hp",
                signal_score=70.0,
                entry_price=100.0,
