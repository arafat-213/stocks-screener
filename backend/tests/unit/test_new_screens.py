...
from app.db.models import Base, Stock, TechnicalSignal, FundamentalCache, SectorSnapshot, ScreenResult
from app.screens.confluence import screen_mtf_confluence, screen_sector_leaders, screen_fresh_52w_breakout
from app.screens.value import screen_qarp, screen_dividend_growth
...
def test_screen_mtf_confluence(db_session):
    ...
    s_d = TechnicalSignal(symbol="RELIANCE", date=date_d, timeframe='D', is_bullish=True, above_200ema=True, rsi=60, entry_score=80)
    s_w = TechnicalSignal(symbol="RELIANCE", date=date_w, timeframe='W', is_bullish=True)
    s_m = TechnicalSignal(symbol="RELIANCE", date=date_m, timeframe='M', is_bullish=True)
    
    # TCS: Bullish on Daily but not Weekly
    t_d = TechnicalSignal(symbol="TCS", date=date_d, timeframe='D', is_bullish=True, above_200ema=True, rsi=60, entry_score=70)
...
def test_screen_sector_leaders(db_session):
    ...
    # Signals
    db_session.add(TechnicalSignal(symbol="TCS", date=date, timeframe='D', rs_score=90, is_bullish=True, above_200ema=True))
    db_session.add(TechnicalSignal(symbol="INFY", date=date, timeframe='D', rs_score=85, is_bullish=True, above_200ema=True))
    db_session.add(TechnicalSignal(symbol="WIPRO", date=date, timeframe='D', rs_score=80, is_bullish=True, above_200ema=True))
    db_session.add(TechnicalSignal(symbol="HCLTECH", date=date, timeframe='D', rs_score=75, is_bullish=True, above_200ema=True))
...
def test_screen_fresh_52w_breakout(db_session):
    ...
    db_session.add(TechnicalSignal(
        ...
        pct_from_52w_high=0.5, volume_breakout=True, 
        above_200ema=True, rsi=65, adx=25, entry_score=90
    ))
    
    # Not a breakout (volume missing)
    db_session.add(TechnicalSignal(
        symbol="TCS", date=date, timeframe='D', 
        pct_from_52w_high=0.5, volume_breakout=False, 
        above_200ema=True, rsi=65, adx=25, entry_score=80
    ))
...
def test_screen_qarp(db_session):
    ...
    db_session.add(FundamentalCache(
        symbol="RELIANCE", roce=0.20, roe=0.18, 
...
def test_sector_rotation_computation(db_session):
    ...
    db_session.add(TechnicalSignal(symbol="TCS", date=date_val, timeframe='D', rs_score=90, is_bullish=True, momentum_3m=10))
    db_session.add(TechnicalSignal(symbol="INFY", date=date_val, timeframe='D', rs_score=80, is_bullish=True, momentum_3m=8))
    db_session.add(TechnicalSignal(symbol="WIPRO", date=date_val, timeframe='D', rs_score=70, is_bullish=False, momentum_3m=6))
    
    db_session.commit()

    compute_sector_rotation(db_session)
    
    snap = db_session.query(SectorSnapshot).filter_by(sector="Tech", date=date_val.date()).first()
...
def test_screen_hot_sectors(db_session):
    ...
    db_session.add(Stock(symbol="TCS", sector="Tech"))
    db_session.add(TechnicalSignal(symbol="TCS", date=date_val, timeframe='D', rs_score=95, is_bullish=True, above_200ema=True, entry_score=90))
    
    db_session.add(Stock(symbol="MARUTI", sector="Auto"))
    db_session.add(TechnicalSignal(symbol="MARUTI", date=date_val, timeframe='D', rs_score=95, is_bullish=True, above_200ema=True, entry_score=90))
