...
def setup_robust_db_mock(mock_db, profitability_streak=True):
    ...
    mock_db.query.return_value.filter_by.return_value.first.return_value = None # For upserts
    mock_db.query.return_value.order_by.return_value.first.side_effect = first_side_effect
...
def test_run_pipeline_tiered_flow(
    ...
    mock_hist = pd.DataFrame({
        ...
        'Close': [100.0, 105.0],
        'Volume': [1000, 1100]
    }, index=pd.to_datetime([datetime.datetime.now() - datetime.timedelta(days=1), datetime.datetime.now()]))
