...
def test_process_symbol_maps_ema_levels():
    ...
    df = pd.DataFrame({"Close": [100, 105]}, index=pd.to_datetime(["2023-01-01", "2023-01-02"]))
    
    with patch('app.pipeline.orchestrator.calculate_combined_score', return_value=ta_data), \
         patch('app.pipeline.orchestrator.fetch_stock_data', return_value=(df, {"longName": "Apple Inc."})), \
        ...
        # Call the function
        signals = process_symbol("AAPL", mock_db, hist=df, info={"longName": "Apple Inc."})
