...
def test_calculate_technical_score_returns_ema_levels():
    ...
    # Actually, calculate_technical_score calls df.ta.ema(length=5, append=True) etc.
    # which APPENDS columns to the dataframe.
    
    # Let's create a DF with enough rows (min 60 bars as per scorer.py logic for 'D' timeframe)
    data = {
        "Open": [100.0] * 65,
        "High": [105.0] * 65,
        "Low": [95.0] * 65,
