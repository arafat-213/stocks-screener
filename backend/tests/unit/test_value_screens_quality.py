...
from app.db.models import Base, TechnicalSignal, FundamentalCache, Stock
...
def test_screen_low_debt_midcap_quality(db_session):
    ...
    assert "FAIL_STREAK.NS" not in symbols # Should fail due to profitability_streak_passed=False
...
def test_screen_undervalued_fundamentals_quality(db_session):
    ...
    assert "PASS.NS" in symbols
    assert "FAIL_DE.NS" not in symbols # Should fail due to de_check_passed=False
    assert "FAIL_STREAK.NS" in symbols # This screen doesn't check profitability_streak_passed (only de_check_passed)
...
def test_screen_steady_compounders_quality(db_session):
    ...
    assert "PASS.NS" in symbols
    assert "FAIL_STREAK.NS" not in symbols # Should fail due to profitability_streak_passed=False
    assert "FAIL_DE.NS" in symbols # This screen doesn't check de_check_passed (only profitability_streak_passed)
