...
const ChangeBanner = ({ changes = [], asOf, prevDate, loading }) => {
  ...
  const newlyBullish = filter(c => ['newly_bullish', 'confluence_improved'].includes(c.change_type), changes);
  const turnedBearish = filter(c => ['turned_bearish', 'confluence_dropped'].includes(c.change_type), changes);
    ...
    <div className="mb-6 overflow-hidden bg-bg-secondary border-2 border-border rounded-2xl shadow-sm">
      ...
      >
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <span className="bg-blue-600 text-white text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full font-black shadow-lg shadow-blue-500/20">{size(changes)}</span>
        </div>
        ...
        <div className="px-4 pb-4 sm:px-6 sm:pb-6 flex flex-col gap-5 sm:gap-6 border-t-2 border-border/50 pt-4 sm:pt-6 animate-fade-in">
          {size(newlyBullish) > 0 && (
            <div className="flex flex-col gap-2.5 sm:gap-3">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-2.5">
                {map(c => <ChangeChip key={c.symbol} item={c} />, newlyBullish)}
              </div>
            </div>
          )}
          ...
          {size(turnedBearish) > 0 && (
            <div className="flex flex-col gap-2.5 sm:gap-3">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-2.5">
                {map(c => <ChangeChip key={c.symbol} item={c} />, turnedBearish)}
              </div>
            </div>
          )}
        </div>
