    ...
    <div className='w-full overflow-x-auto bg-bg-secondary border-2 border-border rounded-2xl mt-6 shadow-sm'>
      <div className='grid min-w-[900px] grid-cols-[140px_110px_100px_90px_80px_90px_90px_1fr]'>
        <div className='contents'>
          <div className='bg-slate-50 dark:bg-slate-900 p-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 border-b-2 border-border sticky top-0 z-[11] left-0'>
            Symbol
          </div>
          <div className='bg-slate-50 dark:bg-slate-900 p-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 border-b-2 border-border sticky top-0 z-10 text-right'>
            Price
          </div>
          <div className='bg-slate-50 dark:bg-slate-900 p-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 border-b-2 border-border sticky top-0 z-10 text-right'>
            Change %
          ...
          <div className='bg-slate-50 dark:bg-slate-900 p-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 border-b-2 border-border sticky top-0 z-10'>
            Sector
          </div>
        </div>
        <div className='contents'>
            ...
            return (
              ...
              >
                ...
                <div className='p-4 px-6 text-sm border-b border-border flex items-center overflow-hidden font-mono font-bold justify-end text-right group-hover:bg-slate-50 dark:group-hover:bg-slate-900 transition-colors'>
                  ₹
                  {stock.close_price?.toLocaleString('en-IN', {
                ...
                <div className='p-4 px-6 text-sm border-b border-border flex items-center overflow-hidden font-mono font-black justify-end text-right group-hover:bg-slate-50 dark:group-hover:bg-slate-900 transition-colors'>
                  <span
                    className={`px-2 py-0.5 rounded shadow-sm ${isPositive ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}
                  >
                    {isPositive ? '+' : ''}
                ...
                <div className='p-4 px-6 text-sm border-b border-border flex items-center overflow-hidden font-mono justify-end text-right font-black group-hover:bg-slate-50 dark:group-hover:bg-slate-900 transition-colors'>
                  <span
                    className={`px-2 py-0.5 rounded ${daily.score >= 70 ? 'bg-green-500 text-white' : daily.score >= 50 ? 'bg-blue-500 text-white' : 'bg-slate-100 dark:bg-slate-800'}`}
                  >
                    {daily.score || '-'}
                  </span>
                </div>
                ...
                <div className='p-4 px-6 text-sm border-b border-border flex items-center overflow-hidden font-mono justify-end text-right group-hover:bg-slate-50 dark:group-hover:bg-slate-900 transition-colors font-bold'>
                  {stock.fundamentals?.roe
                    ? `${(stock.fundamentals.roe * 100).toFixed(1)}%`
                    : '-'}
                </div>
            ...
            (_, i) => (
              <div key={i} className='contents'>
                {mapWithIndex((_, j) => (
                  <div
                    key={j}
                    className={`p-4 px-6 border-b border-border flex items-center ${j === 0 ? 'sticky left-0 z-[2] bg-inherit' : ''}`}
                  >
