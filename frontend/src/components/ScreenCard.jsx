const ScreenCard = ({ screen, isSelected, onClick }) => {
  ...
  const badgeClass = categoryClasses[screen.category.toLowerCase().replace(' ', '-')] || 'bg-slate-500 text-white';
  ...
  return (
    <div 
      ...
      onClick={onClick}
    >
      <span className={`self-start text-[9px] font-black px-2.5 py-1 rounded-lg uppercase tracking-[0.15em] shadow-sm ${badgeClass}`}>
        {screen.category}
      </span>
      <h3 className="text-lg font-black text-text m-0 tracking-tight">{screen.label}</h3>
      <p className="text-xs font-bold text-slate-500 dark:text-slate-400 m-0 line-clamp-2 leading-relaxed uppercase tracking-tight">{screen.description}</p>
    </div>
  );
};
