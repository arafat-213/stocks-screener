...
const SetupBadge = ({ setup }) => {
  ...
  const typeClass =
    typeClasses[setup.setup_type] || 'bg-text-muted/15 text-text-muted';

  return (
    <div
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[0.7rem] font-semibold uppercase tracking-tight ${typeClass}`}
    >
      {label}
    </div>
  );
