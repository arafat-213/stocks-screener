      ...
      <div className='w-full overflow-hidden rounded-2xl border-2 border-border bg-bg-secondary shadow-sm'>
        <div className='flex bg-slate-50 dark:bg-slate-900 border-b-2 border-border'>
          {columns.map((col) => (
            <div
              key={col.key}
              className='flex-1 px-6 py-4 text-left text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]'
            >
              {col.label}
            </div>
          ))}
      ...
      <div className='min-w-full inline-block align-middle'>
        ...
        <div className='flex flex-col'>
          {sortedData.map((row, i) => (
            <div
              key={row.symbol || i}
              className='flex border-b border-border last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors duration-150'
            >
              {columns.map((col) => {
                ...
                return (
                  <div
                    key={col.key}
                    className='flex-1 px-6 py-4 text-sm text-text font-bold flex items-center overflow-hidden'
                  >
                    {col.render ? (
                      col.render(val, row)
                    ) : (
