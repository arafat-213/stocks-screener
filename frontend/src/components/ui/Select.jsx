...
const Select = ({ value, onChange, options, placeholder = "Select option", label, className = "" }) => {
  ...
  return (
    <div className={`relative w-full min-w-[160px] ${className}`} ref={dropdownRef}>
      {label && <label className="block text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 mb-2 tracking-[0.2em]">{label}</label>}
      <div 
        ...
        <div className="absolute top-[calc(100%+8px)] left-0 right-0 z-[1000] max-h-[300px] overflow-y-auto rounded-2xl p-2 shadow-2xl bg-bg-secondary border-2 border-border animate-fade-in">
          {options.map((option) => (
            ...
            >
              ...
              {value === option.value && <Check size={16} className="text-blue-600 dark:text-blue-400" />}
            </div>
          ))}
        </div>
      )}
