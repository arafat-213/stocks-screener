...
import { AlertCircle, Info, ShieldCheck, Clock } from 'lucide-react';
    ...
    <>
      <div className='w-full flex flex-col gap-6 pb-40'>
        <header className='flex justify-between items-start'>
          <div className='flex flex-col'>
            ...
            <p className='text-text-muted'>
              Live strategy verification without real capital
            </p>
          </div>
          ...
          <div className='flex items-center'>
            {pipeline && (
              ...
              >
                ...
                <span>
                  {pipeline?.is_stale
                    ? 'Stale — pipeline not run today'
                    : `Updated ${pipeline?.data_age_hours}h ago`}
                </span>
              </div>
            )}
          </div>
        ...
        <div className='min-h-[400px]'>
          {activeTab === 'overview' && (
            <OverviewView portfolio={portfolio} setPortfolio={setPortfolio} />
          )}
...
const OverviewView = ({ portfolio, setPortfolio }) => {
  ...
  useEffect(() => {
    const p1 = portfolio
      ...
      : getPaperPortfolio();
    Promise.all([p1, getPaperTrades({ limit: 100 })]).then(([p, t]) => {
      setPortfolio(p.data);
      setTrades(t.data);
      setLoading(false);
    });
  ...
  if (portfolio?.status === 'no_portfolio') {
    return (
      <div className='flex flex-col items-center justify-center p-20 bg-bg-secondary border-2 border-dashed border-border rounded-3xl text-center'>
        ...
        <p className='text-text-muted'>
          Run a pipeline cycle to initialize the paper trading portfolio.
        </p>
      </div>
    );
  }
  ...
  const StatCard = ({ label, value, color, comparison, subLabel }) => (
    <div className='flex flex-col gap-1 p-6 bg-bg-secondary border border-border rounded-2xl shadow-sm'>
      ...
      <div
        className={`text-3xl font-black tracking-tight ${color || 'text-text'}`}
      >
        {value}
      </div>
      {comparison !== undefined && (
        <div className='text-[11px] font-medium text-text-muted mt-1'>
          <span className='font-black text-text'>{comparison}</span>{' '}
          {subLabel && <span className='opacity-60'>vs {subLabel}</span>}
        </div>
      )}
    </div>
  );
    ...
    <div className='flex flex-col gap-6'>
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4'>
        <StatCard
          ...
          color={
            portfolio.total_return_pct >= 0 ? 'text-bullish' : 'text-bearish'
          }
      ...
      <div className='grid grid-cols-1 lg:grid-cols-5 gap-6 mt-2'>
        <div className='lg:col-span-3 p-6 bg-bg-secondary border border-border rounded-2xl shadow-sm'>
          <div className='flex flex-col mb-8'>
            <h3 className='text-lg font-black uppercase tracking-tight text-text'>
              Cumulative P&L
            </h3>
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className='p-3 bg-bg-secondary/90 backdrop-blur-md border border-border rounded-xl shadow-xl'>
                          ...
                          <p
                            className={`text-lg font-black ${data.trade.pnl >= 0 ? 'text-bullish' : 'text-bearish'}`}
                          >
                          ...
                          <p className='text-[10px] font-bold text-text-muted uppercase tracking-tight mt-1 border-t border-border pt-1'>
                            Last: {data.trade.symbol} (
                            {data.trade.return_pct.toFixed(1)}%)
                          </p>
                        </div>
                      );
                    }
        ...
        <div className='lg:col-span-2 p-6 bg-bg-secondary border border-border rounded-2xl shadow-sm'>
          <h3 className='text-lg font-black uppercase tracking-tight text-text mb-6'>
            Strategy Drift
          </h3>
          <table className='w-full border-collapse'>
            ...
            <tbody className='text-sm font-medium'>
              ...
              <tr
                className={`border-b border-border/50 ${getDivergenceStyle(portfolio.win_rate, BACKTEST_BENCHMARKS.win_rate)}`}
              >
                <td className='py-4 text-text-muted'>Win Rate</td>
                <td className='py-4 text-right font-black'>
                  {portfolio.win_rate}%
                </td>
                <td className='py-4 text-right font-bold text-text-muted'>
                  {BACKTEST_BENCHMARKS.win_rate}%
                </td>
              </tr>
              <tr
                className={`border-b border-border/50 ${getDivergenceStyle(portfolio.avg_return_pct, 5.4)}`}
              >
                <td className='py-4 text-text-muted'>Avg Return %</td>
                <td className='py-4 text-right font-black'>
                  {portfolio.avg_return_pct}%
                </td>
                <td className='py-4 text-right font-bold text-text-muted'>
                  5.4%
                </td>
              ...
              <tr
                className={`border-b border-border/50 ${getDivergenceStyle(portfolio.profit_factor, BACKTEST_BENCHMARKS.profit_factor)}`}
              >
                ...
                <td className='py-4 text-right font-bold text-text-muted'>
                  {BACKTEST_BENCHMARKS.profit_factor}
                </td>
              </tr>
              <tr>
                <td className='py-4 text-text-muted'>Avg Holding Days</td>
                <td className='py-4 text-right font-black'>
                  {portfolio.avg_holding_days || 0}d
                </td>
                <td className='py-4 text-right font-bold text-text-muted'>
                  {BACKTEST_BENCHMARKS.avg_holding_days}d
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div className='flex flex-col gap-4'>
      <div className='flex items-center gap-3 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl text-text leading-snug'>
        ...
        <p className='text-sm font-medium'>
          These orders will be evaluated at tomorrow's market open. Entry is at
          the close of the bar where price confirms EMA20 support.
        </p>
      </div>
            ...
            <tbody className='text-sm'>
              {size(pending) === 0 ? (
                <tr>
                  <td
                    colSpan='7'
                    className='text-center text-text-muted py-20 font-bold uppercase tracking-widest text-xs opacity-50'
                  >
                    No pending orders being watched.
                  </td>
                </tr>
              ) : (
                map((pos) => {
                  ...
                  if (distance !== null) {
                    if (distance <= 2)
                      distanceClass = 'bg-bullish/10 text-bullish';
                    else if (distance <= 5)
                      distanceClass = 'bg-warning/10 text-warning';
                  }
                  ...
                  return (
                    ...
                    >
                      <td className='p-4'>
                        <div className='font-black text-blue-600 dark:text-blue-400 tracking-tight'>
                          {pos.symbol}
                        </div>
                      ...
                      <td className='p-4 text-text-muted font-medium'>
                        {new Date(pos.signal_date).toLocaleDateString()}
                      </td>
                      <td className='p-4'>
                        <div className='text-[9px] font-black uppercase text-text-muted tracking-widest leading-none mb-1'>
                          touch target
                        </div>
                        <div className='font-mono font-black'>
                          ₹{pos.ema20_at_signal?.toFixed(2) || 'N/A'}
                        </div>
                      </td>
                      <td className='p-4'>
                        <span
                          className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-tight ${distanceClass}`}
                        >
                          {distance !== null
                            ? `${distance.toFixed(1)}% above`
                            : 'Pending'}
                        </span>
                      </td>
                      <td className='p-4'>
                        <div className='flex flex-col gap-1.5 w-24'>
                          <div className='text-[10px] font-bold text-text-muted uppercase'>
                            Day {pos.wait_days} of 8
                          </div>
                          <div className='h-1 bg-border rounded-full overflow-hidden'>
                            <div
                              className='h-full bg-blue-500 transition-all duration-500'
                              style={{ width: `${(pos.wait_days / 8) * 100}%` }}
                            ></div>
                          </div>
                        </div>
                      </td>
                      <td className='p-4'>
                        <div
                          className={`inline-flex px-2 py-0.5 rounded font-black text-xs ${
                            pos.signal_score >= 60
...
const PositionsView = () => {
  ...
  const sortedPositions = useMemo(() => {
    return [...positions].sort((a, b) => {
      if (sortBy === 'pnl')
        return (b.unrealised_pct || 0) - (a.unrealised_pct || 0);
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6'>
        {size(sortedPositions) === 0 ? (
          <div className='md:col-span-2 xl:col-span-3 flex flex-col items-center justify-center p-20 bg-bg-secondary border-2 border-dashed border-border rounded-3xl text-center'>
            <p className='font-bold uppercase tracking-widest text-xs text-text-muted'>
              No open positions. Check pending orders for upcoming entries.
            </p>
          </div>
        ) : (
          map((pos) => {
            ...
            const currentPos = Math.min(
              Math.max(((pos.current_price - pos.stop_loss) / range) * 100, 0),
              100
            );
            const entryPos = ((pos.entry_price - pos.stop_loss) / range) * 100;
            const pnlColor =
              pos.unrealised_pct >= 0 ? 'text-bullish' : 'text-bearish';
            const pnlBg = pos.unrealised_pct >= 0 ? 'bg-bullish' : 'bg-bearish';

            return (
              <div
                key={pos.symbol}
                className='flex flex-col bg-bg-secondary border border-border rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all group'
              >
                <div className='flex justify-between items-start p-6 border-b border-border/50'>
                  <div className='flex flex-col'>
                    <h3 className='text-2xl font-black tracking-tighter text-blue-600 dark:text-blue-400 leading-none mb-1'>
                      {pos.symbol}
                    </h3>
                  ...
                  <div className='text-[10px] font-black uppercase tracking-widest text-text-muted bg-bg-elevated px-2.5 py-1 rounded-full border border-border'>
                    In{' '}
                    {new Date(pos.entry_date).toLocaleDateString(undefined, {
                      month: 'short',
                      day: 'numeric',
                    })}
                  </div>
                ...
                <div className='flex flex-col gap-6 p-6'>
                  <div className='flex justify-between items-end'>
                    <div
                      className={`text-3xl font-black tracking-tight ${pnlColor}`}
                    >
                      {pos.unrealised_pct >= 0 ? '+' : ''}
                      {pos.unrealised_pct?.toFixed(2)}%
                    </div>
                    <div className='flex flex-col items-end'>
                      <div className='text-[9px] font-black uppercase tracking-widest text-text-muted leading-none mb-1'>
                        Unrealised P&L
                      </div>
                      <div
                        className={`text-xl font-black ${pnlColor} tracking-tight`}
                      >
                        ₹
                        {(
                  ...
                  <div className='flex flex-col gap-2'>
                    <div className='relative h-2 rounded-full overflow-hidden flex'>
                      <div
                        className='h-full bg-bearish/20'
                        style={{ width: `${entryPos}%` }}
                      ></div>
                      <div
                        className='h-full bg-bullish/20'
                        style={{ width: `${100 - entryPos}%` }}
                      ></div>
                      <div
                        className='absolute top-0 bottom-0 w-[2px] bg-text shadow-lg z-10'
                        style={{ left: `${entryPos}%` }}
                      ></div>
                      <div
                        className={`absolute top-0 bottom-0 w-3 h-3 -mt-0.5 rounded-full border-2 border-bg-secondary shadow-lg z-20 ${pnlBg}`}
                        style={{
                          left: `${currentPos}%`,
                    ...
                    <div className='flex justify-between text-[10px] font-black tracking-widest text-text-muted uppercase'>
                      ...
                      <span>
                        +
                        {(
                          ((pos.target - pos.entry_price) / pos.entry_price) *
                          100
                        ).toFixed(1)}
                        % TGT
                      </span>
                  ...
                  <div className='grid grid-cols-2 gap-8 pt-2'>
                    <div className='flex flex-col gap-2'>
                      <div className='flex justify-between items-center'>
                        ...
                        <div className='text-[10px] font-black text-text'>
                          Day {pos.holding_days}/50
                        </div>
                      </div>
                      <div className='h-1 bg-border rounded-full overflow-hidden'>
                        <div
                          className='h-full bg-primary'
                          style={{ width: `${(pos.holding_days / 50) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                    <div className='flex flex-col items-end'>
                      <div className='text-[10px] font-black uppercase tracking-widest text-text-muted leading-none mb-1'>
                        Position Size
                      </div>
                      <div className='text-sm font-black tracking-tight text-text'>
                        ₹{pos.position_size.toLocaleString()}
                      </div>
                      <div className='text-[9px] font-bold text-text-muted uppercase tracking-tighter'>
                        {Math.floor(pos.position_size / pos.entry_price)} SHARES
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    {pos.atr_trail_active ? (
                      <div className='flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/10 text-amber-600 rounded-lg border border-amber-500/20 shadow-sm shadow-amber-500/5'>
                        ...
                        <span className='text-[10px] font-black uppercase tracking-wider'>
                          Trailing Active: ₹{pos.highest_price?.toFixed(1)}
                        </span>
                      </div>
                    ) : (
...
const HistoryView = () => {
  ...
  const getReasonStyles = (reason) => {
    switch (reason) {
      ...
      case 'holding_period':
        return 'bg-slate-100 dark:bg-slate-800 text-text-muted';
      default:
            ...
            <tbody className='text-sm font-medium'>
              {trades.map((t, idx) => (
                <tr
                  ...
                  className='border-b border-border/50 hover:bg-bg-elevated/30 transition-colors'
                >
                  <td className='p-4 font-black text-text tracking-tight'>
                    {t.symbol}
                  </td>
                  <td className='p-4'>
                    <div className='text-[10px] font-bold text-text-muted uppercase tracking-tighter'>
                      {new Date(t.entry_date).toLocaleDateString(undefined, {
                        month: 'short',
                        day: 'numeric',
                      })}{' '}
                      →{' '}
                      {new Date(t.exit_date).toLocaleDateString(undefined, {
                        month: 'short',
                        day: 'numeric',
                      })}
                    </div>
                  </td>
                  <td className='p-4 text-text-muted font-black text-xs'>
                    {t.holding_days}d
                  </td>
                  <td className='p-4 font-mono text-xs text-text-muted'>
                    ...
                    → <span>₹{t.exit_price.toFixed(1)}</span>
                  </td>
                  <td
                    className={`p-4 text-right font-black ${t.return_pct >= 0 ? 'text-bullish' : 'text-bearish'}`}
                  >
                    {t.return_pct >= 0 ? '+' : ''}
                    {t.return_pct.toFixed(1)}%
                  </td>
                  <td
                    className={`p-4 text-right font-black ${t.pnl >= 0 ? 'text-bullish' : 'text-bearish'}`}
                  >
                    ₹
                  ...
                  <td className='p-4 text-center'>
                    <span
                      className={`px-2 py-0.5 rounded-[4px] text-[9px] font-black uppercase tracking-widest ${getReasonStyles(t.exit_reason)}`}
                    >
...
const AnalyticsView = () => {
  ...
  useEffect(() => {
    Promise.all([getPaperPortfolio(), getPaperTrades({ limit: 500 })]).then(
      ([p, t]) => {
        setPortfolio(p.data);
        setTrades(t.data);
        setLoading(false);
  ...
  return (
    <div className='flex flex-col gap-8'>
      <section className='p-8 bg-bg-secondary border border-border rounded-3xl shadow-sm'>
        <h3 className='text-xl font-black uppercase tracking-tight text-text mb-8'>
        ...
        <div className='mt-6 p-4 bg-bg-elevated rounded-2xl border border-border'>
          <p className='text-sm font-bold text-text'>
            Paper trading is tracking the backtest equivalent within acceptable
            variance over {portfolio?.total_trades || 0} trades.
          </p>
        </div>
      </section>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        <div className='p-6 bg-bg-secondary border border-border rounded-3xl shadow-sm flex flex-col items-center'>
          <h4 className='text-sm font-black uppercase tracking-widest text-text-muted mb-8'>
            Backtest Exit Breakdown
          </h4>
          <div style={{ width: '100%', height: 220 }}>
            <ResponsiveContainer>
              <PieChart>
                <Pie
                  data={BACKTEST_BENCHMARKS.exit_reasons}
                  innerRadius={65}
                  outerRadius={85}
                  paddingAngle={5}
                  dataKey='value'
                ...
                >
                  {BACKTEST_BENCHMARKS.exit_reasons.map((entry, index) => (
                    <Cell
                      key={index}
                      fill={
                        ['#EF4444', '#3B82F6', '#22C55E', '#94A3B8'][index % 4]
                      }
                    />
                  ))}
                </Pie>
        ...
        <div className='p-6 bg-bg-secondary border border-border rounded-3xl shadow-sm flex flex-col items-center'>
          ...
          <div className='grid grid-cols-2 gap-x-8 gap-y-2 mt-4 w-full px-8'>
            {exitData.length > 0 ? (
              ...
              <div className='col-span-2 text-center text-[10px] font-bold text-text-muted uppercase tracking-widest opacity-50 py-4'>
                No completed trades
              </div>
            )}
          </div>
      ...
      <div className='flex flex-col gap-3'>
        {(() => {
          const paperStopPct =
            exitData.find((d) => d.name.toLowerCase() === 'stop loss')?.value ||
            0;
          const backtestStopPct =
            BACKTEST_BENCHMARKS.exit_reasons.find((d) => d.name === 'Stop Loss')
              ?.value || 0;
          const paperHoldPct =
            exitData.find((d) => d.name.toLowerCase() === 'holding period')
              ?.value || 0;
          const backtestHoldPct =
            BACKTEST_BENCHMARKS.exit_reasons.find((d) => d.name === 'Held')
              ?.value || 0;
              ...
              {paperStopPct > backtestStopPct + 10 && (
                <div className='p-4 bg-warning/5 border border-warning/20 rounded-2xl flex gap-3 items-start'>
                  ...
                  <p className='text-sm font-medium leading-relaxed'>
                    <span className='font-black text-warning uppercase tracking-widest text-[10px] mr-2'>
                      Advisory:
                    </span>
                    More stops than expected. Consider whether current market
                    conditions favour a tighter regime filter or whether entries
                    are occurring in lower-quality setups.
                  </p>
                </div>
              )}
              {paperHoldPct > backtestHoldPct + 5 && (
                <div className='p-4 bg-warning/5 border border-warning/20 rounded-2xl flex gap-3 items-start'>
                  ...
                  <p className='text-sm font-medium leading-relaxed'>
                    ...
                    More positions expiring without hitting stop or target.
                    Momentum may not be developing after entry — review sector
                    conditions.
                  </p>
                </div>
              )}
      ...
      <section className='p-8 bg-bg-secondary border border-border rounded-3xl shadow-sm'>
        <h4 className='text-lg font-black uppercase tracking-tight text-text mb-8'>
  ...
  return (
    <div className='fixed bottom-[64px] lg:bottom-0 left-0 lg:left-[280px] right-0 h-auto lg:h-20 p-3 lg:p-0 flex flex-col lg:flex-row justify-between items-center z-[60] bg-bg-secondary/95 backdrop-blur-xl border-t border-border shadow-[0_-8px_30px_rgb(0,0,0,0.12)] max-w-full overflow-hidden'>
      <div className='flex flex-col items-center lg:items-start lg:pl-10 mb-2 lg:mb-0'>
        <div className='text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-text-muted leading-none mb-1.5 lg:mb-2'>
        ...
        <div className='flex gap-1.5'>
          {dots.map((active, i) => (
            <div
              key={i}
              className={`w-2 h-2 lg:w-2.5 lg:h-2.5 rounded-full transition-all duration-700 ${
                active
                  ? 'bg-bullish shadow-[0_0_10px_rgba(34,197,94,0.4)] scale-110'
                  : 'bg-border'
              }`}
            ></div>
          ))}
      ...
      <div className='flex flex-col items-center flex-1 w-full px-2 lg:px-0'>
        ...
        <div className='text-[8px] lg:text-[9px] font-bold text-text-muted uppercase tracking-widest mt-1 opacity-60 text-center'>
          Targets: {BACKTEST_BENCHMARKS.win_rate}% Win ·{' '}
          {BACKTEST_BENCHMARKS.profit_factor} PF
        </div>
      </div>

      <div className='lg:pr-10 mt-2 lg:mt-0 w-full lg:w-auto px-4 lg:px-0'>
        <div
          className={`px-4 py-1.5 lg:px-5 lg:py-2 rounded-xl text-[9px] lg:text-xs font-black uppercase tracking-widest border transition-all text-center ${
            filledCount === 5
