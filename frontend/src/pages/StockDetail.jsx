...
const StockDetail = () => {
  ...
  const renderScoreCard = (tf, label) => {
    ...
    return (
      <div className="bg-bg-secondary rounded-xl p-6 border-2 border-border shadow-sm hover:border-blue-500/30 transition-colors" key={tf}>
        <h3 className="text-[11px] text-slate-500 dark:text-slate-400 mb-4 uppercase tracking-[0.2em] font-black">{label} Timeframe</h3>
        <div className="flex justify-between items-center">
          <div className={`text-5xl font-black ${scoreData.score >= 70 ? 'text-green-500' : scoreData.score >= 50 ? 'text-blue-500' : 'text-text'}`}>
            {scoreData.score?.toFixed(1) || scoreData.score}
          </div>
          <div className="text-right flex flex-col gap-2">
            <div className={`inline-block px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wider ${isBullish ? 'bg-green-500 text-white shadow-lg shadow-green-500/20' : 'bg-red-500 text-white shadow-lg shadow-red-500/20'}`}>
              {scoreData.ema_signal}
            </div>
            <div className="text-[13px] font-mono font-bold text-slate-500 dark:text-slate-400">
              RSI: <span className={scoreData.rsi <= 30 ? 'text-green-500' : scoreData.rsi >= 70 ? 'text-red-500' : 'text-text'}>{scoreData.rsi?.toFixed(1)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start mb-10 gap-6 sm:gap-0 mt-4 lg:mt-0">
        <div className="flex flex-col gap-2 w-full sm:w-auto">
          <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
            <h1 className="text-3xl sm:text-5xl m-0 text-text font-black tracking-tighter">{symbol.replace('.NS', '')}</h1>
            <span className="bg-blue-500 text-white px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-black uppercase tracking-widest shadow-lg shadow-blue-500/20">{sector}</span>
          </div>
          <div className="text-lg sm:text-xl text-slate-500 dark:text-slate-400 font-medium">{name}</div>
        </div>
        <div className="text-left sm:text-right flex flex-col gap-1.5 sm:gap-2 w-full sm:w-auto">
          <div className="text-3xl sm:text-5xl font-black font-mono text-text tracking-tighter">₹{latestOhlc.close.toLocaleString('en-IN')}</div>
          <div className={`text-lg sm:text-xl font-black flex items-center gap-2 sm:justify-end ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
        ...
        <div className="flex flex-col gap-8">
          <section className="bg-bg-secondary rounded-2xl p-8 border-2 border-border shadow-sm">
            ...
            <div className="h-[400px] lg:h-[600px] w-full rounded-xl overflow-hidden border border-border">
              <CandlestickChart data={ohlcv} isDark={isDark} containerHeight={600} />
            </div>
          </section>
        ...
        <div className="flex flex-col gap-6 lg:sticky lg:top-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <section className="bg-bg-secondary rounded-2xl p-6 border-2 border-border shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 uppercase font-black">RS Score</span>
                  <span className="text-lg font-black text-blue-600 dark:text-blue-400">{dailyScore?.rs_score?.toFixed(1) || 'N/A'}</span>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 uppercase font-black">ADX</span>
                  <span className="text-lg font-black text-text">{dailyScore?.adx?.toFixed(1) || 'N/A'}</span>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 uppercase font-black">Mom. (1m)</span>
                  <span className={`text-lg font-black ${(dailyScore?.momentum_1m || 0) >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                    {dailyScore?.momentum_1m ? `${(dailyScore.momentum_1m).toFixed(1)}%` : 'N/A'}
                  </span>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 uppercase font-black">Resistance</span>
                  <span className="text-lg font-black text-text">
                    {dailyScore?.resistance ? `₹${dailyScore.resistance.toFixed(1)}` : 'N/A'}
                  </span>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border col-span-2">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-2 uppercase font-black">52W High/Low Range</span>
                  <div className="flex items-center justify-between font-mono font-black">
                    <span className="text-green-500 text-sm">H: ₹{dailyScore?.week52_high?.toLocaleString('en-IN') || 'N/A'}</span>
                    <div className="flex-1 mx-4 h-1 bg-slate-200 dark:bg-slate-800 rounded-full relative">
                        <div className="absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-blue-500 rounded-full shadow-lg shadow-blue-500/50" style={{ left: '60%' }}></div>
                    </div>
                    <span className="text-red-500 text-sm">L: ₹{dailyScore?.week52_low?.toLocaleString('en-IN') || 'N/A'}</span>
                  </div>
                </div>
                {dailyScore?.volume_breakout && (
                   <div className="bg-blue-600 p-4 rounded-xl border border-blue-700 col-span-2 flex items-center justify-between shadow-lg shadow-blue-500/20 animate-pulse">
                      <span className="text-[10px] text-white uppercase font-black tracking-widest">Volume Breakout</span>
            ...
            <div className="bg-bg-secondary rounded-2xl p-6 border-2 border-border shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 uppercase font-black">P/E Ratio</span>
                  <span className="text-lg font-black text-text">{fundamentals.pe?.toFixed(1) || 'N/A'}</span>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 uppercase font-black">ROE %</span>
                  <span className={`text-lg font-black ${fundamentals.roe > 0.15 ? 'text-green-500' : 'text-text'}`}>{fundamentals.roe ? `${(fundamentals.roe * 100).toFixed(1)}%` : 'N/A'}</span>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 uppercase font-black">D/E Ratio</span>
                  <span className={`text-lg font-black ${fundamentals.debt_equity > 1.5 ? 'text-red-500' : 'text-text'}`}>{fundamentals.debt_equity?.toFixed(2) || 'N/A'}</span>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border">
                  <span className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 uppercase font-black">Market Cap</span>
                  <span className="text-lg font-black text-text truncate">{formatMarketCap(fundamentals.market_cap)}</span>
                </div>
              </div>
            </div>
            ...
            {dailyScore && (
              <div className="bg-bg-secondary rounded-2xl p-6 border-2 border-border shadow-sm">
                <h3 className="text-[11px] text-slate-500 dark:text-slate-400 mb-6 uppercase tracking-[0.2em] font-black">Score Attribution</h3>
