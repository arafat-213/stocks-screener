...
const EmptyWatchlistState = () => (
  <div className='bg-bg-secondary p-20 rounded-3xl border-2 border-dashed border-border flex flex-col items-center justify-center text-center'>
    ...
    <p className='text-text-muted text-sm mt-1 max-w-xs'>
      Add stocks from the dashboard to track them for high-probability entries.
    </p>
    <Link
      to='/'
      className='mt-6 px-6 py-3 bg-blue-600 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20'
    >
      Go to Dashboard
    </Link>
  </div>
...
const Watchlist = () => {
  ...
  const augmentedItems = useMemo(() => {
    ...
    return map((item) => {
      const signal = new Date(item.signal_date);
      const days_elapsed = Math.floor((today - signal) / (1000 * 60 * 60 * 24));
      return { ...item, days_elapsed };
    })(items);
  }, [items]);
  ...
  const handleStatusUpdate = useCallback(
    async (symbol, status) => {
      try {
        ...
        setItems((prev) =>
          map((item) => (item.symbol === symbol ? { ...item, status } : item))(
            prev
          )
        );
      } catch (err) {
  ...
  const handleDelete = useCallback(
    async (symbol) => {
      ...
      try {
        ...
        setItems((prev) => filter((item) => item.symbol !== symbol)(prev));
      } catch (err) {
  ...
  const columns = useMemo(
    () => [
      // ... (rest of the columns definition stays the same, omitted for brevity in replace tool but I should include the whole block if I'm replacing it)
      {
        key: 'symbol',
        label: 'Symbol',
        sortable: true,
      ...
      },
      {
        ...
        render: (val) => (
          <div className='flex items-center gap-1.5'>
            <Clock
              size={14}
              className={val > 5 ? 'text-amber-500' : 'text-slate-400'}
            />
            <span className={val > 8 ? 'text-red-500 font-black' : ''}>
              {val}d
            </span>
          </div>
        ),
          ...
          return (
            <div className='flex flex-col gap-0.5'>
              <span className='text-[10px] text-slate-500 uppercase font-bold tracking-tighter'>
                ₹{row.planned_entry_low.toFixed(1)} - ₹
                {row.planned_entry_high.toFixed(1)}
              </span>
        ...
        render: (_, row) => {
          ...
          return (
            <div className='flex flex-col gap-0.5'>
              ...
              <span
                className={`text-[10px] font-bold ${diff < 1 ? 'text-green-600' : 'text-slate-500'}`}
              >
                {diff > 0 ? '+' : ''}
                {diff.toFixed(2)}% vs EMA
              </span>
            </div>
          );
        },
    ...
    <div className='w-full animate-fade-in'>
      ...
      <header className='mb-10'>
        ...
        <p className='text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-[10px]'>
          Active setups awaiting entry or monitoring.
        </p>
      </header>
